...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/18 18:11:47 by hasalam           #+#    #+#             */
/*   Updated: 2023/05/20 03:13:25 by hasalam          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void	julia_helper2(t_fract *juls, t_map *mapin)
{
	mapin->x = juls->x0;
	mapin->min_in = juls->x1;
	mapin->max_in = juls->y1;
	juls->a = map(mapin) * juls->zoom;
	mapin->x = juls->y0;
	mapin->min_in = juls->x2;
	mapin->max_in = juls->y2;
	juls->b = map(mapin) * juls->zoom;
	juls->iter = 0;
...
void	julia_helper1(t_fract *juls)
{
	t_map	mapin;
	...
	while (juls->y0 <= 500)
	{
		...
		while (juls->x0 <= 500)
		{
			julia_helper2(juls, &mapin);
			juls->x0++;
		}
		juls->y0++;
	}
	mlx_put_image_to_window(juls->mlx, juls->mlx_win, juls->img, 0, 0);
	mlx_key_hook(juls->mlx_win, key_hook, juls);
	mlx_hook(juls->mlx_win, 17, 0, ft_close, juls);
	mlx_loop(juls->mlx);
}
